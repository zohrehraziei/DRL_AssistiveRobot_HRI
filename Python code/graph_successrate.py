...
with open('Drink-PPO-1-9-21.csv', newline='\n') as csvfile:
        ...
        all_lines = list(spamreader)
        last_line = all_lines[-1]
        multiplier = float(last_line[1])
...
with open('Feed-SAC-1-9-21.csv', newline='\n') as csvfile:
        ...
        for row in all_lines:
                ...
                prevRow = row
                idx = idx + 1
datasrate = {'PPO': [maxRewardDrinkPPO,maxRewardFeedPPO],
        'SAC': [maxRewardDrinkSAC,maxRewardFeedSAC]
        }

dfsrate = pd.DataFrame(datasrate,columns=['PPO','SAC'], index = ['Drink Success Rate','Feed Success Rate',])

plt.style.use('ggplot')

dfsrate.plot.barh()

plt.title('Compare Success Rate PPO vs SAC')
plt.ylabel('')
plt.xlabel('')
plt.show()
